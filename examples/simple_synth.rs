...
fn main() -> Result<()> {
    ...
    let midi_res = plugin.send_midi(&events)?;
    println!("✓ MIDI note on events sent with result: {midi_res:?}");
    ...
    for i in 0..num_buffers {
        ...
        let (rms_left, rms_right, peak_left, peak_right) =
            analyze_buffer(&left_out.clone(), &left_out);
