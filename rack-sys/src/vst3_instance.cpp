  ...
  while (*utf16_str && processed < MAX_STRING_LENGTH) {
    ...
    if (c < 0x80) {
      ...
      if (low >= 0xDC00 && low <= 0xDFFF) {
        ...
        // Max valid codepoint is 0x10FFFF (which is exactly 0x10000 + (0x3FF <<
        // 10) + 0x3FF)
        if (codepoint > 0x10FFFF) {
          // Overflow - invalid codepoint (should never happen with valid
          // surrogates)
          result.append("\xEF\xBF\xBD"); // UTF-8 replacement character U+FFFD
        } else {
      ...
      // Malformed: Low surrogate (U+DC00 to U+DFFF) without preceding high
      // surrogate
      result.append("\xEF\xBF\xBD"); // UTF-8 replacement character U+FFFD
    } else {
...
private:
  uint32
      ref_count_; // Non-atomic - IMPLEMENT_REFCOUNT macro handles thread-safety
...
RackVST3Plugin *rack_vst3_plugin_new(const char *path, const char *uid) {
  ...
  if (plugin->component->initialize(
          FUnknownPtr<IHostApplication>(new HostApplication())) != kResultOk) {
    // Component creation succeeded but initialization failed - no need to
    // terminate IPtr will automatically release when plugin is deleted
  ...
  if (plugin->component->getControllerClassId(controllerCID) == kResultTrue) {
    ...
    if (plugin->controller) {
      ...
      if (plugin->controller->initialize(FUnknownPtr<IHostApplication>(
              new HostApplication())) != kResultOk) {
...
int rack_vst3_plugin_initialize(RackVST3Plugin *plugin, double sample_rate,
  ...
  if (numInputBuses > 0) {
    ...
    BusInfo busInfo;
    if (plugin->component->getBusInfo(kAudio, kInput, 0, busInfo) ==
        kResultOk) {
  ...
  if (numOutputBuses > 0) {
    ...
    BusInfo busInfo;
    if (plugin->component->getBusInfo(kAudio, kOutput, 0, busInfo) ==
        kResultOk) {
  ...
  if (plugin->controller) {
    ...
    for (int32 i = 0; i < param_count; ++i) {
      ...
      if (plugin->controller->getParameterInfo(i, vst3_param_info) ==
          kResultOk) {
        RackVST3Plugin::ParameterInfo info;
        info.id = vst3_param_info.id;

        // Convert UTF-16 to UTF-8 (proper conversion for international
        // characters)
    ...
    for (int32 i = 0; i < program_list_count; ++i) {
      ...
      if (unit_info->getProgramListInfo(i, list_info) == kResultOk) {
        ...
        for (int32 j = 0; j < list_info.programCount; ++j) {
          ...
          if (unit_info->getProgramName(list_info.id, j, program_name) ==
              kResultOk) {
...
int rack_vst3_plugin_reset(RackVST3Plugin *plugin) {
  ...
  // Without this, another thread could change initialized between check and
  // lock
  std::lock_guard<std::mutex> lock(g_vst3_lifecycle_mutex);
...
int rack_vst3_plugin_process(RackVST3Plugin *plugin, const float *const *inputs,
  ...
  if (num_output_channels !=
      static_cast<uint32_t>(plugin->num_output_channels)) {
    return RACK_VST3_ERROR_INVALID_PARAM;
  }
  // Frame count must not exceed the max block size configured during
  // initialization
...
StringArray rack_vst3_plugin_get_subcategories(RackVST3Plugin *plugin) {
  ...
  std::vector<VST3::Hosting::ClassInfo> infos =
      plugin->module->getFactory().classInfos();

  for (VST3::Hosting::ClassInfo info : infos) {
    // std::cout << "info.category " << info.category() << std::endl;
    std::vector<std::string> tmpSubCate = info.subCategories();

    for (std::string category : tmpSubCate) {
      subCategories.push_back(category);
      // std::cout << "sub_category " << category << std::endl;
    }
  }

  // std::cout << "All subCategories" << std::endl;
  ...
  char **c_strs = new char *[subCategories.size()];

  for (int i = 0; i < subCategories.size(); i++) {
    c_strs[i] = new char[subCategories[i].size() + 1];
    strncpy(c_strs[i], subCategories[i].c_str(), subCategories[i].size());
    c_strs[i][subCategories[i].size()] = 0;
  }

  // return subCategories;
  return {c_strs, subCategories.size()};
}
...
int rack_vst3_plugin_set_parameter(RackVST3Plugin *plugin, uint32_t index,
  ...
  IParamValueQueue *queue =
      plugin->input_param_changes.addParameterData(param_id, queue_index);
  if (queue) {
...
int rack_vst3_plugin_parameter_info(RackVST3Plugin *plugin, uint32_t index,
  ...
  // Min/max/default
  if (min)
    *min = static_cast<float>(param_info.min_value);
  if (max)
    *max = static_cast<float>(param_info.max_value);
  if (default_value)
    *default_value = static_cast<float>(param_info.default_value);
...
int rack_vst3_plugin_load_preset(RackVST3Plugin *plugin,
                                 int32_t preset_number) {
  ...
  if (preset_number < 0 ||
      preset_number >= static_cast<int32_t>(plugin->presets.size())) {
    return RACK_VST3_ERROR_NOT_FOUND;
  }
  ...
  if (program_data) {
    ...
    //   - IPtr(..., false) takes ownership WITHOUT calling addRef() (false =
    //   don't addRef)
    //   - IPtr destructor calls release() when going out of scope
    //   - This ensures cleanup on ALL code paths (success, error, early
    //   return)
    IPtr<MemoryStream> stream(new MemoryStream(), false);

    // Get program data
    tresult result = program_data->getProgramData(preset.program_list_id,
                                                  preset.program_index, stream);
    if (result == kResultOk) {
      // Reset stream and apply the preset data
      stream->seek(0, IBStream::kIBSeekSet, nullptr);
      result = program_data->setProgramData(preset.program_list_id,
                                            preset.program_index, stream);
  ...
  for (int32 i = 0; i < param_count; i++) {
    ...
    // Only consider discrete parameters (stepCount > 0) or parameters with
    // kIsProgramChange flag
    bool is_discrete = param_info.stepCount > 0;
    bool is_program_change =
        (param_info.flags & ParameterInfo::kIsProgramChange) != 0;
    ...
    // - Check for word boundaries to avoid matching "programming",
    // "unpresettable", etc.
    ...
    if (is_program_param) {
      ...
      if (plugin->controller->setParamNormalized(param_id, normalized_value) ==
          kResultOk) {
  ...
  // Fallback 2: Try using IUnitInfo to select the unit containing this
  // program Some plugins may respond to unit selection by loading the first
  // program in that unit
  ...
  // No fallback succeeded - plugin doesn't support programmatic preset
  // loading This is a plugin limitation, not a runtime error
...
int rack_vst3_plugin_get_state_size(RackVST3Plugin *plugin) {
  ...
  stream->write(&size_marker_placeholder, sizeof(size_marker_placeholder),
                nullptr);
...
int rack_vst3_plugin_get_state(RackVST3Plugin *plugin, uint8_t *data,
  ...
  // IPtr ensures automatic cleanup on ALL code paths (success, error, buffer
  // size check failure)
  IPtr<MemoryStream> stream(new MemoryStream(), false);
  ...
  int64 component_end_pos = 0;
  stream->tell(&component_end_pos);
  uint32_t component_state_size =
      static_cast<uint32_t>(component_end_pos - component_start_pos);
...
int rack_vst3_plugin_set_state(RackVST3Plugin *plugin, const uint8_t *data,
  ...
  // Read component state size marker first (written at position 0 during
  // serialization)
  uint32_t component_state_size = 0;
  int32 bytes_read = 0;
  tresult result = stream->read(&component_state_size,
                                sizeof(component_state_size), &bytes_read);

  if (result != kResultOk || bytes_read != sizeof(component_state_size)) {
    return RACK_VST3_ERROR_GENERIC;
  }

  // Set component state (reads from current position, right after size
  // marker)
...
int rack_vst3_plugin_send_midi(RackVST3Plugin *plugin,
  ...
  for (uint32_t i = 0; i < event_count; ++i) {
    ...
    case 0x90: // Note On
      ...
      vst3_event.noteOn.velocity =
          static_cast<float>(midi_event.data2) / 127.0f;
    ...
    case 0xA0: // Polyphonic Key Pressure (Aftertouch)
      ...
      vst3_event.polyPressure.pressure =
          static_cast<float>(midi_event.data2) / 127.0f;
      plugin->input_events.addEvent(vst3_event);
      break;
    ...
    case 0xC0: // Program Change
      ...
      vst3_event.type = Event::kLegacyMIDICCOutEvent;
      vst3_event.midiCCOut.channel = midi_event.channel;
      vst3_event.midiCCOut.controlNumber =
          0x80; // >= 0x80 indicates non-CC MIDI
    ...
    case 0xD0: // Channel Pressure (Aftertouch)
      ...
      vst3_event.midiCCOut.channel = midi_event.channel;
      vst3_event.midiCCOut.controlNumber =
          0x81; // >= 0x80 indicates non-CC MIDI
      vst3_event.midiCCOut.value = midi_event.data1;
      vst3_event.midiCCOut.value2 = 0;
      plugin->input_events.addEvent(vst3_event);
      break;
    ...
    case 0xE0: { // Pitch Bend
      ...
      // Use LegacyMIDICCOutEvent with controlNumber >= 0x80 for non-CC MIDI
      // Note: Not all plugins may support this encoding
      // The plugin should interpret value (LSB) and value2 (MSB) as 14-bit
      // pitch bend
      vst3_event.type = Event::kLegacyMIDICCOutEvent;
      vst3_event.midiCCOut.channel = midi_event.channel;
      vst3_event.midiCCOut.controlNumber =
          0x82; // >= 0x80 indicates non-CC MIDI
